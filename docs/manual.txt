_*Check out the printable PDF version of this manual:*_ http://spellout.net/latest_ibex_manual.pdf
...
files/directories; the setup described below is just the simplest.
    ...
    # Ensure that the `www` directory is somewhere within the root directory of your HTTP server. You may want to place the entire `webspr` directory within the root directory, or you may want to copy the files in `www` to a new location within the root directory. All of these files are ordinary static files except for `server.py`, which needs to be run as a CGI app.

    # Decide on a location for the main webspr directory. Depending on the details of your setup, you may be able to leave it in place, or you may need to copy it to a directory which the HTTP server has permission to access (so that `server.py` may access files in this directory when it is executed).

    # Edit `server.py` and change the value of `SERVER_CONF_PY_FILE` to point to the new location of server_conf.py.

    # Edit `server_conf.py` and change the value of `IBEX_WORKING_DIR` to the new location of the main `webspr` dir.

    # Edit `server_conf.py` and change change the value of `SERVER_MODE` to `"cgi"`.

    # If the version of Python you wish to use is not the default used by the HTTP server, add a `#!` line at the beginning of `server.py`.

    # You may need to rename `server.py` (e.g. some servers may only execute CGI scripts with the extension `.cgi`). If this is the case, follow the instructions in the subsection below.
...
|| _vboxCallbackWhenChildFinishes || `null` || If set, this function is called every time one of the VBox's children finishes. It is called with two paramaters: the index of the child, and an array of arrays (= array of lines) giving the results for that child. ||
...
be displayed in the same location, add a `<label>` with 'for' set to "__ALL_FIELDS__".
